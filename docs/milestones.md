...
| M4 | Dashboard, history, print records and currency-separated totals | Implemented; local checks and browser journey pass. Hosted CI unverified. |
| M5 | Hosting, production email, backups, privacy and launch operations | Planned |

M1 targets AC01 and the agency ownership portion of AC02. M2 completes AC03 and extends AC02 to clients and projects. M3 implements AC04–AC12, request isolation under AC02, and local email-failure handling for AC15. M4 implements print records and dashboard totals (AC13–AC14) and extends AC02 to history and print reads. Production email operations remain M5. No interviews or demand validation have been performed.
...
Limitations: hosted CI and Docker Compose startup remain unverified. The browser journey used the same browser profile for owner and client navigation; API tests separately assert that client review does not use owner authentication. Review-code email used local Mailpit only. Test output includes existing Starlette/httpx and Alembic configuration deprecation warnings. The mounted checkout still requires a native persistent filesystem for PostgreSQL data outside the disposable `/tmp` path. At the M3 checkpoint, dashboard totals, print records, production email, and deployment remained for later milestones.

M4 verification on 2026-09-25:

- A fresh isolated PostgreSQL 18 test database upgraded through Alembic head. All **17 PostgreSQL integration tests passed**, including the empty and populated upgrade paths and two new M4 tests for tenant isolation, request filters, event pagination, exact currency totals above JavaScript's safe integer limit, effective expiry without read mutations, immutable snapshots after contact edits, and client review-session access.
- Backend Ruff lint and format checks passed. Frontend ESLint, TypeScript, Node money tests, and Vite build passed. No M4 migration was needed.
- A local browser journey created an owner and agency, issued a request, opened owner history and print view, verified the client through Mailpit, approved, and opened the client print record. The dashboard showed the approved request under restored status/currency filters. At 375×812 it had no horizontal overflow; print media hid controls while retaining the record.

Limitations: hosted CI and Docker Compose startup remain unverified. Browser owner and client checks used one browser profile; separate API tests check that unverified client access cannot read a proposal and foreign owners cannot read history. Browser printing was checked using print-media emulation, without saving a physical PDF. Client print access lasts only as long as the existing review session. Production email and deployment remain M5. Existing Starlette/httpx and Alembic deprecation warnings remain. The local database used disposable `/tmp` storage for this verification.
