...
export function ReviewPage() {
  ...
  return <div className="flex min-h-screen flex-col"><header className="flex items-center justify-between gap-4 border-b border-line bg-white px-[clamp(20px,5vw,72px)] py-[23px]"><span className="inline-flex items-center gap-3 whitespace-nowrap text-[.96rem] font-bold tracking-[-.025em] text-ink no-underline hover:text-ink">Agency Change Approval</span><span className="text-[.85rem] text-muted max-[600px]:hidden">Client review</span></header><main className="w-full flex-1 px-5 py-[clamp(30px,7vw,80px)]"><div className="mx-auto w-full max-w-[1120px] "><h1>Review additional work</h1><section className="max-w-[760px] rounded-2xl border border-line bg-white p-[clamp(24px,3vw,36px)] shadow-[0_8px_28px_#203d3108] max-[600px]:px-[18px] max-[600px]:py-[22px] [&_form+form]:mt-9 [&_form+form]:border-t [&_form+form]:border-line [&_form+form]:pt-6 [&_section+section]:mt-[30px]">{loading ? <p role="status">Loading review…</p> : <><Message notice={notice} />{access && !proposal && <><p>Designated recipient: <strong>{access.masked_email}</strong></p><p>Review window closes {timeText(access.expires_at)}.</p>{access.status === 'pending' ? <><p>Verify access to this email before the proposal is shown. A forwarded link alone does not grant access.</p><button disabled={busy} onClick={sendCode}>Send verification code</button><form onSubmit={verify}><label>Six-digit verification code<input name="code" inputMode="numeric" pattern="[0-9]{6}" autoComplete="one-time-code" required /></label><button disabled={busy}>Verify and view proposal</button></form></> : <p>This request is {access.status}. Contact the agency if you need a new request.</p>}</>}{proposal && <><p>Status: <strong>{proposal.status}</strong> · Review expires {timeText(proposal.expires_at)}</p><ProposalView snapshot={proposal.snapshot} />{proposal.status === 'pending' ? <><p className="mb-6 leading-[1.55] text-muted">The additional charge is the total agreed extra amount. Tax and billing are handled externally. Email verification confirms mailbox access, not corporate authority.</p><form onSubmit={e => decide(e, 'approved')}><h2>Approve</h2><label>Your name<input name="name" maxLength={120} required /></label><label className="flex items-start gap-3 [&_input]:mt-1 [&_input]:min-h-0 [&_input]:w-auto"><input name="agreement" type="checkbox" required />I agree to the displayed work, additional price, and proposed delivery date.</label><button disabled={busy}>Approve request</button></form><form onSubmit={e => decide(e, 'rejected')}><h2>Reject</h2><label>Your name<input name="name" maxLength={120} required /></label><label>Reason (optional)<textarea name="reason" maxLength={1000} rows={3} /></label><button className="!border-line !bg-white !text-accent hover:!bg-[#edf5f1]" disabled={busy}>Reject request</button></form></> : <><p role="status">{proposal.decided_at ? `Decision recorded for ${proposal.decision_name} (${proposal.decision_email}) at ${timeText(proposal.decided_at)}.${proposal.decision_reason ? ` Reason: ${proposal.decision_reason}` : ''}` : `This request is ${proposal.status}.`}</p><a href={`/review/print?request_id=${proposal.id}`}>Print or save record</a></>}</>}</>}</section></div></main><footer>Client review</footer></div>
}
