...
import { ClientProposalView, ProposalView, timeText } from './ProposalView'
...
export function PrintPage({ kind, id }: { kind: 'owner' | 'client'; id?: string }) {
  ...
  return <div className="mx-auto max-w-[960px] p-4 sm:p-6 print:max-w-none print:p-0 print:bg-white print:font-serif print:text-[9.5pt] print:leading-[1.3] print:text-black print:[&_button]:hidden print:[&_a]:hidden print:[&_h1]:text-[18pt] print:[&_h2]:text-[12pt] print:[&_h3]:text-[10pt] print:[&_p]:leading-[1.3]">
    ...
    </main> : <main className="block break-words border border-line bg-white p-[30px] max-[600px]:p-5 print:border-0 print:p-0 [&_dt]:break-inside-avoid [&_dd]:break-inside-avoid [&_li]:break-inside-avoid [&_h2]:break-after-avoid print:[&_dd]:mb-[10pt]"><h1>Change request record</h1><p>Request ID: {record.id}<br />Project ID: {record.project_id}</p><p><strong>Status: {record.status}</strong></p><dl><dt>Issued</dt><dd>{record.issued_at ? timeText(record.issued_at) : '—'}</dd><dt>Review expires</dt><dd>{record.expires_at ? timeText(record.expires_at) : '—'}</dd>{record.decided_at && <><dt>Decision</dt><dd>{timeText(record.decided_at)} · {record.decision_name} ({record.decision_email}){record.decision_reason ? ` · Reason: ${record.decision_reason}` : ''}</dd></>}</dl><ProposalView snapshot={record.snapshot} />{record.status === 'expired' && <p>Review window ended at {record.expires_at && timeText(record.expires_at)}. Any expiry event below records when that status was saved.</p>}{data?.events && <EventHistory events={data.events} />}<p className="mb-6 leading-[1.55] text-muted">The baseline is the agency’s record of the original agreement. Tax and billing are handled externally. Email verification confirms mailbox access, not corporate authority. This is an application record, not an external certification.</p></main>)}
  </div>
}
