...
function IssuedRequest({ record }: { record: ChangeRequest }) {
  ...
  return <><p>Status: <strong>{current.status}</strong>{current.expires_at && ` · Review expires ${timeText(current.expires_at)}`}</p>{current.status === 'pending' && <p className="mb-6 leading-[1.55] text-muted">Pending means awaiting a decision; it does not confirm delivery or reading.</p>}<Message notice={notice} />{current.snapshot && <ProposalView snapshot={current.snapshot} />}{current.decided_at && <p>Decision by {current.decision_name} ({current.decision_email}) at {timeText(current.decided_at)}{current.decision_reason ? ` · Reason: ${current.decision_reason}` : ''}</p>}<div className="mt-6 flex flex-wrap items-center gap-3 max-[600px]:[&_button]:flex-1"><a href={`/requests/${record.id}/print`}>Print record</a>{current.status === 'pending' && <><button disabled={busy} onClick={() => action('rotate-link')}>Replace lost link</button><button className="!border-line !bg-white !text-accent hover:!bg-[#edf5f1]" disabled={busy} onClick={() => action('withdraw')}>Withdraw request</button></>}{current.status !== 'pending' && <button className="!border-line !bg-white !text-accent hover:!bg-[#edf5f1]" disabled={busy} onClick={() => action('duplicate')}>Duplicate as draft</button>}</div>{link && <LinkBox link={link} />}</>
}
...
export function RequestsPage({ id }: { id: string }) {
  ...
  return <Layout><div className="mx-auto w-full max-w-[1120px] "><h1>{id === 'new' ? 'New change request' : 'Change request'}</h1>{feedback}{data && <><p><a href={`/projects/${data.project.id}`}>Back to project</a></p><section className="max-w-[760px] rounded-2xl border border-line bg-white p-[clamp(24px,3vw,36px)] shadow-[0_8px_28px_#203d3108] max-[600px]:px-[18px] max-[600px]:py-[22px] [&_form+form]:mt-9 [&_form+form]:border-t [&_form+form]:border-line [&_form+form]:pt-6 [&_section+section]:mt-[30px]">{!data.record || data.record.status === 'draft' ? <RequestEditor record={data.record} project={data.project} /> : <IssuedRequest record={data.record} />}</section>{data.record?.linked_from_id && <p>Duplicated from <a href={`/requests/${data.record.linked_from_id}`}>earlier request</a>.</p>}{data.history && <section className="max-w-[760px] rounded-2xl border border-line bg-white p-[clamp(24px,3vw,36px)] shadow-[0_8px_28px_#203d3108] max-[600px]:px-[18px] max-[600px]:py-[22px] [&_form+form]:mt-9 [&_form+form]:border-t [&_form+form]:border-line [&_form+form]:pt-6 [&_section+section]:mt-[30px]"><EventHistory events={data.history} />{data.record?.status === 'expired' && <p>Review window ended at {timeText(data.record.expires_at!)}. Expiry may appear in history later when saved by a write.</p>}</section>}</>}</div></Layout>
}
