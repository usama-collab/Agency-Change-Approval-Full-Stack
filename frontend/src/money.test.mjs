...
const { parsePrice, priceText, aggregatePriceText } = await import(`data:text/javascript;base64,${Buffer.from(compiled).toString('base64')}`)
...
test('format agency totals beyond the safe integer boundary exactly', () => {
  assert.equal(aggregatePriceText('9007199254752501'), '90071992547525.01')
  assert.equal(aggregatePriceText('0'), '0.00')
})
