...
function ProjectForm({ project, clients }: { project?: Project; clients: Client[] }) {
  ...
  if (project?.first_issued_at) return <><p className="mb-6 leading-[1.55] text-muted">This baseline is the agency’s record of the original agreement. It is locked because a request has been issued.</p><dl><dt>Client</dt><dd>{clients.find(c => c.id === project.client_id)?.name}</dd><dt>Project</dt><dd>{project.title}</dd><dt>Baseline deliverables</dt><dd>{project.baseline_deliverables}</dd><dt>Exclusions</dt><dd>{project.exclusions || 'None'}</dd><dt>Original price and date</dt><dd>{project.currency} {priceText(project.original_price_minor)} · {project.delivery_date}</dd><dt>Current agreed total and date</dt><dd>{project.currency} {priceText(project.current_price_minor)} · {project.current_delivery_date}</dd></dl></>
  return <><p className="mb-6 leading-[1.55] text-muted">This baseline is your record of an existing agreement. It does not record client approval in this product. Tax and billing are handled externally. The baseline locks after the first request is issued.</p><Message notice={notice} />{preview ? <section aria-label="Review project"><h2 tabIndex={-1} ref={node => node?.focus()}>Review project</h2><dl><dt>Client</dt><dd>{clients.find(c => c.id === preview.client_id)?.name}</dd><dt>Title</dt><dd>{preview.title}</dd><dt>Baseline deliverables</dt><dd>{preview.baseline_deliverables}</dd><dt>Exclusions</dt><dd>{preview.exclusions || 'None'}</dd><dt>Original price</dt><dd>{preview.currency} {priceText(preview.original_price_minor)}</dd><dt>Delivery date</dt><dd>{preview.delivery_date}</dd></dl><div className="mt-6 flex flex-wrap items-center gap-3 max-[600px]:[&_button]:flex-1"><button disabled={busy} onClick={() => save(preview)}>{busy ? 'Saving…' : 'Save project'}</button><button disabled={busy} className="!border-line !bg-white !text-accent hover:!bg-[#edf5f1]" onClick={() => setPreview(null)}>Back to edit</button></div></section> : <form onSubmit={submit}>
...
function ProjectDetail({ id }: { id: string }) {
  ...
  const { data, feedback } = useLoad(load)
  return <>{feedback}{data && <><ProjectForm {...data} />{data.project && <section><h2>Agreed and pending additions</h2><dl><dt>Approved additions</dt><dd>{data.project.currency} {priceText(data.project.current_price_minor - data.project.original_price_minor)}</dd><dt>Current agreed total and deadline</dt><dd>{data.project.currency} {priceText(data.project.current_price_minor)} · {data.project.current_delivery_date}</dd><dt>Pending addition</dt><dd>{data.project.currency} {priceText(data.pending?.items[0]?.additional_price_minor || 0)}</dd></dl><h2>Change requests</h2><p><a href={`/requests/new?project_id=${data.project.id}`}>New change request</a></p>{!data.requests?.total ? <p>No requests yet.</p> : <><ul className="list-none p-0 [&_li]:grid [&_li]:gap-2 [&_li]:break-words [&_li]:border-b [&_li]:border-line [&_li]:py-[18px] [&_span]:text-[.9rem] [&_span]:text-muted">{data.requests.items.map(r => <li key={r.id}><a href={`/requests/${r.id}`}>{r.description}</a><span>{r.status} · {data.project!.currency} {priceText(r.additional_price_minor)}{r.expires_at ? ` · Expires ${timeText(r.expires_at)}` : ''}</span></li>)}</ul><Pagination offset={requestOffset} total={data.requests.total} onChange={setRequestOffset} /></>}</section>}</>}</>
}
export function ProjectsPage({ id }: { id?: string }) {
  return <Layout><div className="mx-auto w-full max-w-[1120px] "><h1>{id === 'new' ? 'Create project' : id ? 'Project baseline' : 'Projects'}</h1><p className="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-[.9rem]"><a href="/projects">All projects</a><a href="/projects/new">Create project</a></p><section className="max-w-[760px] rounded-2xl border border-line bg-white p-[clamp(24px,3vw,36px)] shadow-[0_8px_28px_#203d3108] max-[600px]:px-[18px] max-[600px]:py-[22px] [&_form+form]:mt-9 [&_form+form]:border-t [&_form+form]:border-line [&_form+form]:pt-6 [&_section+section]:mt-[30px]">{id ? <ProjectDetail id={id} /> : <ProjectList />}</section></div></Layout>
}
